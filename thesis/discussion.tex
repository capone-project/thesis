...
\tikzset{
    network/.style={draw, cloud, cloud puffs=20, minimum height=3cm, minimum width=3cm},
    router/.style={draw=black, fill=white, rectangle, minimum height=1cm, minimum width=3mm},
    service/.style={draw, rectangle},
    display/.style={draw, ellipse},
    controller/.style={draw, circle},
...
\begin{figure}
    ...
    \begin{tikzpicture}
        \node[service,minimum ] (c1) {c1};
        \node[controller, below right=of c1] (controller) {mp};
        \node[network, fit={(c1) (controller)}, label=above:Corporate network] (d1) {};
        \node[router] at (d1.east) (r1) {};

        \node[service, right=3cm of d1] (c2) {c2};
        \node[network, fit={(c2)}, label=above:Home network] (d2) {};
        \node[router] at (d2.west) (r2) {};

        \draw[invocation] (controller.east) -- ($(r1) - (0,0.2)$) -- node[below] {1.}  ($(c2.west) - (0,0.2)$);
        \draw[invocation] (controller.west) -- node[below left] {2.} (c1.south);
        \draw[invocation] (c1.east) -- ($(r1) + (0,0.2)$) -- node [above] {3.} ($(c2.west) + (0,0.2)$);
    \end{tikzpicture}
