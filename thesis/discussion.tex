...
As it seems, a big grief with initial implementations of inetd was that it was not possible to have proper rate-limiting and thus protect the resources of servers that it runs on.
...
Especially when considering the setting of Intern of Things, where a plethora of devices might be present, this mode of operation becomes interesting.
...
Regarding flexibility, SLP is another interesting protocol which is able to perform peer-to-peer discovery, but to also scale up in the presence of directory agents.
...
Our implementation only considers the key exchange between two participants, as there are never more than two parties directly communicating with each other inside the same session.
...
In our protocol, we have chosen to perform the initial key exchange via a key agreement protocol based on long-term signature keys.
...
All these schemes are based on the Diffie-Hellman problem \cite{diffie1976new}.
...
It is essentially a three-pass protocol based on the Diffie-Hellman problem and public key signatures with added key confirmation.
...
Extending this scheme, we arrive at structures like the Web of Trust, which is commonly used e.g. for GnuPG \cite{gnupg}.
Given an entity a user wants to connect to and which is currently unknown to himself, the user will connect to known users whom he trusts and asks them to retrieve the identity for S.
...
The two persistent end-points provide the interface to query a service in order to obtain additional information on the service's specifics, and to request a new session within a service.
...
If the process is now allowed to access this segment, the operating systems grant it access to the peripheral device.
...
In distributed systems, though, these capabilities are not usable due to the common heterogeneity of devices attached.
...
\begin{quote}
    The principal failing of capabilities is the difficulty of administration.
...
While not implemented right now, we envision a different way of propagating capabilities these capabilities in entirely decentralized environments without having to request them from servers.
...
Nevertheless, this allows us to iterate further on the design of capabilities, as has been presented for example for the capability propagation scheme presented above.
...
The access control lists contain, for each user who is allowed to connect, an entry with the user's long-term public signature key.
...
So while we unfortunately employ both kinds of access control in the protocol, they are cleanly separated by the objects they actually handle.
...
The mechanism of TGTs requires further thought about the revocation of tickets.
...
Assuming an adversary hosting a server on the local network, he may try to generate a key pair whose public key has only a small Levenshtein distance to another server on the network he wishes to impersonate.
...
Assuming the Invoke service is hosted on his office's work station, he simply directs it to invoke the display service located at the conference room and tap ``Invoke'', upon which both servers connect to each other.
...
A recurring theme with mobile clients is that they are inherently less safe than servers located at one's home or in datacenters.
It is much easier to lose or have stolen a mobile phone or tablet than to lose a full desktop computer.
Even when the device has full disk encryption enabled, unlocking the contents is usually only a 4-digit pin code away.
