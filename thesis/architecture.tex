...
Server A, which is owned by Alice, is running a graphical application and hosts a single Invoke service.
...
Like this, we are able to improve security as compared to sending package lengths in plain text.
...
On initial connection the client per definition has a nonce of value $0$ and the server has a nonce of value $1$.
...
This naive approach is not even guaranteed to work for two hosts running on the same architecture, as the compiler may for example choose to re-arrange structure entries.
...
\begin{description}
    \item[GenerateSid()]\hfill\\
        ...
        The session identifier is used to distinguish sessions from each other in case multiple connection establishment protocols are currently in use.
...
That is given a client and a certain amount of services that all reside on the same network, we want to get to know all the services that are present without previously knowing which machines host them at which network address.
...
Periodic announcements of the service conceal the intent of clients to discover local services, as these clients do not need to broadcast their intent to the network and only passively consume announcement messages.
...
While this might not be problematic when only a few services are available on a certain subnet, it may become impractical when hundreds of services are present which are constantly broadcasting announcement messages to each member of the network.
...
Just how important this distinction is becomes particularly clear when we consider the process how users want to start interacting with a certain service.
Assume Alice starts up her mobile phone and intents to discover a certain service she expects to be available on the local network.
...
We will discuss this topic in more detail in section \ref{sec:key-exchange}.
...
\begin{description}
    ...
    \item[List of known identities]\hfill\\
        ...
        The list's purpose is to suppress a server's announcement in case it is already known to the client.
...
\begin{description}
    ...
    \item[Category]\hfill\\
        ...
        Assuming a user Alice wants to connect to a service providing input devices, she may easily filter the list of local services by restricting them to the category ``Input'', only.
    \item[Port]\hfill\\
...
After the initial service discovery, we only have access to the most basic information about each server.
...
\begin{itemize}
    ...
    \item Details about certain services provided by a server may contain sensitive information.
...
\begin{description}
    ...
    \item[Version]\hfill\\
        ...
        If for example a new version of the application backing the service plugin is released which is incompatible with previous versions, we may need to increment the plugin's version in order to guarantee interoperability.
...
When either session owner or session creator now wishes to terminate the session, they will send a session termination message to this end-point (see appendix \ref{apx:session-termination}).
